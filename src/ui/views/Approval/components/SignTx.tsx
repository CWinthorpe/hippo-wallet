...
const SignTx = ({
  ...
  const handleCoboArugsConfirm = async (account: Account) => {
    ...
    if (
      ...
      // #4083 stale-approval guard adopted; the global Cobo account restore
      // it calls is a Hippo round-9 removal (delegation is request-scoped
      // via sendRequest {account}, no singleton slot exists to reset).
      return;
    }
  ...
  // Hippo privacy invariant: the upstream security-rule audit reporter that
  // #4083 re-gated with isSecurityReady stays purged here as in prior rounds
  // — the rule audit list is never sent to any remote API (round-6 purge).
  // logId.current is retained solely for explain/reporting identity fields.

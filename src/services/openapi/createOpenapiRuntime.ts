...
class UIOpenapiStore implements OpenapiClientStore {
  ...
  applyPublicState(
    ...
    // Hippo privacy policy: PublicOpenapiStore carries no installation
    // identity, so UI state can only ever change `host`.
  ...
  // Hippo privacy policy: the installation identifier is never present in a
  // UI context. Reads are pinned to null and any write from library code is
  // discarded, so an X-API-Key header can never be attached from a page.
  get apiKey(): string | null {
    return null;
  }

  set apiKey(_apiKey: string | null) {
    // discarded by design
  }

  get apiTime(): number | null {
    return null;
  }

  set apiTime(_apiTime: number | null) {
    // discarded by design
  }

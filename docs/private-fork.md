# Hippo Wallet: privacy and RPC routing

Hippo Wallet permanently disables the upstream wallet's behavioral telemetry. It does **not** remove every upstream-operated feature API. Do not describe the current build as backend-independent or as making no third-party requests.
...
Chrome MV3 builds also ship `rules/privacy.json`, which blocks requests to Google Analytics, DeBank Matomo, Sentry, and the upstream uninstall endpoint as a defense-in-depth backstop. The tracking preference is compile-time locked to opt-out and cannot be re-enabled from storage or UI.

## Remaining operational disclosures

The wallet still relies on `api.rabby.io` and related upstream/DeBank infrastructure for product functionality. Depending on the feature, those requests can include wallet addresses, dapp origins, unsigned transaction intent, simulation inputs, security checks, token/portfolio queries, and backend-managed transaction data. This fork clears the upstream API client's persistent per-installation `X-API-Key`/creation-time headers, but the service operator can still correlate functional requests through submitted addresses, origins, transaction data, IP address, timing, and other ordinary network metadata. Static asset requests expose the same ordinary network metadata. The screenshot-feedback upload path remains source-compatible but is not presented as Hippo Wallet support.
...
A future strict `rpc-only` mode would need to disable or replace the upstream analysis and portfolio APIs and visibly accept reduced decoding, simulation, risk-warning, token, and portfolio coverage.
...
A signed raw transaction is submitted to exactly one endpoint. Its returned transaction hash must be a 32-byte JSON-RPC hash. When the default profile selects direct RPC broadcast, failure is surfaced and is not retried through the upstream backend. Profiles that are backend-managed still use that backend as their sole destination; configure a custom broadcast RPC to avoid that path. To use MEV Blocker for protected broadcast, configure it explicitly as the broadcast RPC rather than relying on a silent fallback.
...
Without that environment variable the extension still builds, but WalletConnect pairing intentionally refuses to initialize instead of using the upstream project identity.

...
describe('notificationService approval identity', () => {
  const makeApproval = (
    ...
    id,
    // Hippo's mandatory identity guard compares the approval's lifecycle
    // epoch (gpt56 round-10); fixtures must carry the service's current
    // epoch exactly like requestApproval stamps it.
    approvedEpoch: notificationService.approvalEpoch,
    taskId: null,
    winProps: {},
    data: {
      approvalComponent,
  ...
  test('resolves exactly the displayed request and rejects reuse against the next queued request', async () => {
    ...
    // Hippo resolveApproval attaches background-derived handshake lineage
    // (__approvalId/__approvalComponent/__signingContext) to the payload so
    // a waiting child cannot forge the parent binding (round-10 blocker 3).

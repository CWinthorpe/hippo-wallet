...
describe('approval identity static constraints', () => {
  ...
  test('ConnectContent awaits resolveApproval before completing', () => {
    // Upstream #4100 settle-before-close invariant, Hippo shape: the perps
    // invite callback is a removed product surface, so ConnectContent's
    // settlement must simply be awaited (never fire-and-forget) so a
    // post-unmount mounted-check cannot drop a legit settlement.
    const source = read(
      path.join(
        SRC_ROOT,

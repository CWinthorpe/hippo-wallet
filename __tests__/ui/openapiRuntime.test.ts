...
jest.mock('@rabby-wallet/rabby-api', () => {
  return {
    OpenApiService: class MockOpenApiService {
      ...
      setAPIKey = jest.fn(async (key: string) => {
...
describe('OpenAPI runtime (Hippo host-only UI store)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockInstances.splice(0);
  });

  test('hydrates host only and keeps UI-side installation identity pinned null', async () => {
    ...
    expect(mockStore.apiKey).toBeNull();
    expect(mockStore.apiTime).toBeNull();
    ...
    // Identity writes from UI code are discarded; no commit is issued and the
    // store getters keep reporting null, so an X-API-Key can never be built.
    await runtime.openapi.setAPIKey('ui-rotated-key');
    expect(commit).not.toHaveBeenCalledWith(
      expect.objectContaining({ apiKey: 'ui-rotated-key' })
    );
    ...
    onUpdate?.({
      origin: 'background-2',
      revision: 1,
      partials: { apiKey: 'ignored-attempt' },
    } as any);
